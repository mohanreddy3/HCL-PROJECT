...
public class MohanProject {

    private static final Logger logger = LoggerFactory.getLogger(MohanProject.class);

    private FrameGrabber frameGrabber;
    private OpenCVFrameConverter.ToMat toMatConverter = new OpenCVFrameConverter.ToMat();
    private volatile boolean running = false;

    private Mohanfacedetector faceDetector = new Mohanfacedetector();
    private AgedetectionCNN ageDetector = new AgedetectionCNN();
    private GenderdetectionCNN genderDetector = new GenderdetectionCNN();
     ...
     * Grabbed and processed frames will be displayed in the
     * {@link #videoPanel}
     */
    public void start() {
        ...
        while (running) {
            try {
                ...
                detectedFaces.entrySet().forEach(rectMatEntry -> {
                    ...
                    GenderdetectionCNN.Gender gender = genderDetector.predictGender(rectMatEntry.getValue(), frame);
    ...
    public static void main(String[] args) {
        MohanProject MohanProject = new MohanProject();

        logger.info("Starting javacv example");
        new Thread(MohanProject::start).start();
